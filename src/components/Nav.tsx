    ...
    <nav className={'nav js-nav' + (scrolled ? ' scrolled' : '')} data-edit-section="nav">
      <div className="nav-inner">
        ...
        <div className="nav-links">
          {/* About is its own page now; the rest are sections of the home page,
              so off /) they need the path in front of the hash. */}
          <a href="/about" className={route === '/about' ? 'active' : ''} data-edit-id="nav.links.about" data-edit-key="nav.about">{t('nav.about')}</a>
          <a href={sectionHref(route, 'diadromi')} data-edit-id="nav.links.journey" data-edit-key="nav.journey">{t('nav.journey')}</a>
          <a href={sectionHref(route, 'programmata')} data-edit-id="nav.links.programs" data-edit-key="nav.programs">{t('nav.programs')}</a>
        </div>
